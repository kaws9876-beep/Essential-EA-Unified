...
.pl{font-size:10px;color:var(--gold2);background:transparent;border:none;cursor:pointer;font-weight:600;letter-spacing:.05em;text-transform:uppercase;font-family:'DM Sans',sans-serif;transition:color .15s}
.pl:hover{color:var(--blk)}
.pb{padding:14px 18px}
.kpi-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:22px}
.kpi{background:var(--warm);border:1px solid var(--tan);border-top:2px solid var(--tan);border-radius:10px;padding:16px 18px;transition:all .2s;cursor:default}
...
.ai-lbl{font-size:9px;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:var(--gold);margin-bottom:10px;display:flex;align-items:center;gap:6px}
...
.d-task{border-radius:3px;padding:6px 8px;margin-bottom:5px;border-left:2px solid var(--tan);background:var(--cream);transition:transform .15s;cursor:default}
...
.pw-note{padding:12px 18px;border-top:1px solid var(--tan);font-size:12px;color:var(--mid);line-height:1.7;background:rgba(200,169,106,.03)}
.pw-note strong{color:var(--gold2);font-weight:500}
.plan-result{background:var(--warm);border:1px solid var(--tan);border-radius:10px;padding:18px;margin-top:14px;animation:popIn .25s ease}
.plan-result pre{white-space:pre-wrap;font-family:'DM Sans',sans-serif;font-size:13px;color:var(--blk);line-height:1.75}
...
.triage-btn:hover{background:var(--blk3)}
.triage-btn:disabled{opacity:.5;cursor:not-allowed}
.t-result{background:var(--blk);border-radius:10px;padding:16px 18px;margin-bottom:18px;animation:popIn .25s ease}
...
.badge-d{background:rgba(74,122,80,.1);color:var(--grn)}
...
.inbox-list{background:var(--warm);border-right:1px solid var(--tan);overflow-y:auto;display:flex;flex-direction:column;-webkit-overflow-scrolling:touch}
...
.mt-defer{background:rgba(168,120,48,.1);color:var(--amb)}
...
.d-ghost:hover{border-color:var(--blk);color:var(--blk)}
.d-primary{background:var(--blk);color:#FAF8F4}
.d-primary:hover{background:var(--blk3)}
...
.ar-pill{font-size:8.5px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;padding:3px 9px;border-radius:9px;cursor:pointer;transition:all .15s;font-family:'DM Sans',sans-serif}
.arp-h{background:rgba(200,169,106,.12);color:var(--gold);border:1px solid rgba(200,169,106,.2)}
.arp-h:hover{background:rgba(200,169,106,.22)}
.arp-d{background:rgba(74,122,80,.12);color:#6AAA70;border:1px solid rgba(74,122,80,.2)}
.arp-d:hover{background:rgba(74,122,80,.22)}
.arp-f{background:rgba(168,120,48,.1);color:#C8963A;border:1px solid rgba(168,120,48,.2)}
.arp-f:hover{background:rgba(168,120,48,.2)}
.detail-body{flex:1;overflow-y:auto;padding:16px 20px;font-size:13px;color:#6A6860;line-height:1.85;-webkit-overflow-scrolling:touch}
.detail-body::-webkit-scrollbar{width:3px}
.detail-body::-webkit-scrollbar-thumb{background:var(--tan2)}
.opt-box{background:var(--warm);border-radius:4px;padding:12px;margin-bottom:8px;border:1px solid var(--tan);cursor:pointer;font-size:12.5px;color:var(--mid);transition:border-color .15s;line-height:1.55}
...
.reply-input{flex:1;background:var(--cream);border:1px solid var(--tan);border-radius:4px;padding:9px 12px;resize:none;font-family:'DM Sans',sans-serif;font-size:13px;color:var(--blk);outline:none;transition:border-color .15s;min-height:40px}
.reply-input:focus{border-color:var(--gold)}
.reply-input::placeholder{color:var(--mid)}
.reply-send{background:var(--blk);color:#FAF8F4;border:none;border-radius:4px;padding:9px 16px;cursor:pointer;font-family:'DM Sans',sans-serif;font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;transition:all .15s;white-space:nowrap;flex-shrink:0}
.reply-send:hover{background:var(--blk3)}
...
.r-btn{flex:1;padding:9px 4px;background:var(--cream);border:1px solid var(--tan);border-radius:4px;color:var(--mid);cursor:pointer;transition:all .15s;font-family:'DM Sans',sans-serif;font-size:12px;text-align:center}
...
.fi2:focus,.fa2:focus{border-color:var(--gold);box-shadow:0 0 0 3px rgba(200,169,106,.1)}
.fa2{resize:vertical;min-height:90px}
.sub-btn{width:100%;padding:12px 24px;background:var(--blk);color:#FAF8F4;border:none;border-radius:4px;font-weight:600;cursor:pointer;transition:all .15s;font-size:13.5px;font-family:'DM Sans',sans-serif}
.sub-btn:hover{background:var(--blk3)}
...
@media(max-width:768px){
  ...
  .day-grid{grid-template-columns:repeat(5,minmax(130px,1fr));overflow-x:auto;-webkit-overflow-scrolling:touch}
  ...
  .d-ghost{display:none}
  .ops-grid{grid-template-columns:1fr}
  .s-grid{grid-template-columns:1fr}
  .tb-date{display:none}
}
@media(max-width:400px){.kpi-grid{grid-template-columns:1fr 1fr;gap:8px}.kv{font-size:1.4rem}.content{padding:12px}}
</style>
</head>
<body>
...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sb-nav">
    ...
    <div class="sb-item" onclick="nav('triage',this)"><span class="sb-icon">🔮</span>Crystal Ball Triage<span class="sb-badge" id="tc">—</span></div>
...
<main class="main">
  ...
  <div class="content" id="content">
        ...
        <div class="cw">
          <div class="panel">
            ...
            <div class="pb">
              ...
              <div class="pi"><div class="pg2">🔮</div><div class="pib"><div class="pit">Review Q1 commission report with CFO</div><div class="pin">Crystal ball · Financial stewardship cadence — monthly</div></div><div class="ptm">2:00 PM</div></div>
              <div class="pi"><div class="pb3">🎾</div><div class="pib"><div class="pit">Schedule team standup for next week</div><div class="pin">Bouncy ball · Delegated to EA — no action needed from you</div></div><div class="ptm">EA owned</div></div>
              <div class="pi"><div class="pb3">🎾</div><div class="pib"><div class="pit">Order closing gift for the Rodriguez family</div><div class="pin">Bouncy ball · Gift preferences logged · EA processing</div></div><div class="ptm">EA owned</div></div>
              <div class="pi"><div class="pg2">🔮</div><div class="pib"><div class="pit">Approve listing photos — 2847 Elmwood</div><div class="pin">Crystal ball · Your eye and your standard · Non-delegatable</div></div><div class="ptm">4:00 PM</div></div>
            </div>
          </div>
          <div class="ai-box">
            <div class="ai-lbl"><span>✦</span> EA AI Insight · This week</div>
            <div class="ai-txt">Your calendar shows <strong>6.5 hours of meeting time before 10am on Tuesday and Wednesday.</strong> Based on your Priority Week, your highest-revenue activity is scheduled for those same windows. Your EA recommends shifting team admin meetings to Thursday afternoon to protect your prime selling hours.</div>
            <div class="ai-acts">
        ...
        <div class="col">
          <div class="panel">
            ...
            <div class="pb" style="padding-top:10px">
              <div class="tl-item"><div class="tl-time">8:00</div><div class="tl-dot dot-grn"></div><div><div class="tl-n">EA Daily Brief</div><div class="tl-s">AI-generated · reviewed</div></div></div>
              ...
              <div class="tl-item"><div class="tl-time">4:00</div><div class="tl-dot dot-grn"></div><div><div class="tl-n">Photo approval</div><div class="tl-s">EA has prepped all assets</div></div></div>
          ...
          <div class="panel">
            <div class="ph"><span class="pt">Weekly Scorecard</span><button class="pl">Full report</button></div>
            <div class="pb">
              <div class="ring-wrap">
                <div class="ring">
    ...
    <div class="screen" id="screen-priorityweek">
      ...
      <div class="pg-s2">Your AI-powered week — built from your goals, your time blocks, and your methodology.</div>
      <div class="pw-layout">
        ...
        <div>
          <div class="week-card" id="week-preview">
            ...
            <div class="pw-note"><strong>✦ EA Note:</strong> Generate your plan above to see your personalized week based on your goals and time blocks.</div>
          </div>
      ...
      <div class="inbox-wrap" id="inbox-wrap">
        <div class="inbox-list">
          ...
          <div id="msg-list">
            ...
            <div class="msg" data-tag="ea" onclick="openMsg(5,this)"><div class="msg-header"><span class="msg-from">NAR Events</span><span class="msg-when">Last wk</span></div><div class="msg-sub">April conference — booking confirmation</div><div class="msg-prev">Your registration and hotel are confirmed for April 14–16…</div><div class="msg-tags"><span class="mtag mt-ea">EA Booked</span></div></div>
          </div>
        </div>
        <div class="msg-detail" id="msg-detail">
          ...
          <div class="detail-body" id="d-body">
            ...
            <div style="margin-top:20px;padding-top:16px;border-top:1px solid #E8E2D8">
              ...
              <div class="opt-box" onclick="setReply('Hi Marcus, great news — let me reach out to the listing agent this morning about the $618K + closing costs position. Will have an update by noon. — Kristina')"><div class="opt-label">OPTION A — Move forward</div>Confirm position, contact listing agent, hold Tuesday walkthrough.</div>
              <div class="opt-box" onclick="setReply('Marcus — I want to review the inspection report before responding. Can we connect at 9:30am tomorrow for 15 minutes? — Kristina')"><div class="opt-label">OPTION B — Review first</div>Request inspection report before negotiating. Schedule call.</div>
            </div>
          </div>
    ...
    <div class="screen" id="screen-operations">
      ...
      <div class="ops-grid">
        <div class="ops-card"><div class="ops-icon">👥</div><div class="ops-title">Team & Pipeline</div><div class="ops-desc">Manage team members, roles, and deal pipeline.</div></div>
        ...
        <div class="ops-card"><div class="ops-icon">🎁</div><div class="ops-title">Gifting & Database</div><div class="ops-desc">Sphere touchpoints, closing gifts, relationship cadence.</div></div>
      </div>
    ...
    <div class="screen" id="screen-settings">
      ...
      <div class="s-grid">
        <div class="s-card"><div class="s-icon">⚙️</div><div class="s-title">Preferences</div><div class="s-desc">Customize notifications and EA behavior.</div></div>
      ...
      <div class="panel">
        ...
        <div class="pb">
          ...
          <div class="fg"><label class="fl2">Your Feedback *</label><textarea id="fb-msg" class="fa2" placeholder="What's working well? What could be improved?"></textarea></div>
          <button class="sub-btn" onclick="submitFB()">Send Feedback ✉️</button>
...
function tCard(t) {
  ...
  return '<div class="t-item"><div class="ti-icon">'+(t.emoji||(t.classification==='crystal'?'🔮':'🎾'))+'</div><div class="ti-body"><div class="ti-task">'+(t.description||t.task||'')+'</div><div class="ti-reason">'+(t.reason||'')+'</div></div><div class="ti-badge '+bc+'">'+bt+'</div></div>';
}
...
const msgs = [
  { sub:'Counter offer — 2847 Elmwood Dr', from:'Marcus Chen', time:'Today 9:14 AM', route:'<strong>Crystal Ball — Needs You.</strong> Counter offer requires your direct judgment. EA has prepared two response approaches.', ptype:'crystal',
    body:'<p style="margin-bottom:14px">Hi Kristina,</p><p style="margin-bottom:14px">They came down to $624,000. Still $11K above but there\'s room. Inspection contingency ends Friday.</p><p>Can you reach out to the listing agent today?</p><p style="margin-top:14px">Thanks, Marcus</p><div style="margin-top:20px;padding-top:16px;border-top:1px solid #E8E2D8"><div class="opt-sep">✦ EA-prepared response options</div><div class="opt-box" onclick="setReply(\'Hi Marcus — let me reach out about the $618K + closing costs position. Update by noon. — Kristina\')"><div class="opt-label">OPTION A — Move forward</div>Confirm position, contact listing agent.</div><div class="opt-box" onclick="setReply(\'Marcus — I want to review the inspection report first. Can we connect at 9:30am tomorrow? — Kristina\')"><div class="opt-label">OPTION B — Review first</div>Request inspection report. Schedule call.</div></div>' },
  { sub:'Referral partner meeting request', from:'Sarah Kim — Lender', time:'Today 8:52 AM', route:'<strong>Bouncy Ball — EA Triaging.</strong> Inbound vendor meeting request. EA is evaluating against your referral criteria.', ptype:'bouncy',
    body:'<p style="margin-bottom:14px">Hi Kristina,</p><p>I\'d love 20 minutes to explore a referral partnership. Available this week or next?</p><p style="margin-top:14px">Best, Sarah</p>' },
  { sub:'Closing confirmed — gift needed by Friday', from:'Rodriguez Closing', time:'Tuesday', route:'<strong>Bouncy Ball — EA Owned.</strong> EA has confirmed the closing and is processing the gift order. No action required.', ptype:'bouncy',
    body:'<p style="margin-bottom:14px">Title confirmed Thursday 2pm for the Rodriguez family.</p><p>EA has selected the Luxury Home Welcome Box. Budget used: $185 of your $200 allowance.</p><p style="margin-top:14px">No action needed. — Your EA</p>' },
  { sub:'Weekly scorecard — Action required', from:'Team Standup Bot', time:'Monday', route:'<strong>Crystal Ball — Needs Your Input.</strong> Two team members missed targets. Only you can address performance accountability.', ptype:'crystal',
    body:'<p style="margin-bottom:14px">Two agents completed fewer than 60% of committed tasks. Pattern has appeared for 2 consecutive weeks.</p><p>EA recommends a 15-min 1:1 with each. Would you like your EA to schedule these?</p>' },
  { sub:'Quote renewal — Q2 supplies', from:'Office Supplies Vendor', time:'Monday', route:'<strong>Bouncy Ball — EA Replied.</strong> Routine vendor communication. EA replied per your vendor protocol. No action needed.', ptype:'bouncy',
    body:'<p style="margin-bottom:14px">Your EA replied on your behalf:</p><p style="background:#FAF8F4;padding:12px;border-radius:4px;border-left:2px solid #C8A96A;margin-bottom:14px">Thanks for the Q2 pricing. We\'ll review and be in touch. — EA on behalf of Kristina</p><p style="color:#8A8880">No action needed from you.</p>' },
  { sub:'April conference — booking confirmation', from:'NAR Events', time:'Last week', route:'<strong>Bouncy Ball — EA Booked.</strong> Conference registration and hotel confirmed per your travel preferences.', ptype:'bouncy',
    body:'<p style="margin-bottom:14px">Registration and hotel confirmed for April 14–16.</p><p>Hotel: Marriott Downtown · Check-in April 13. All details added to your calendar.</p>' }
];
...
async function submitFB() {
  ...
  try {
    ...
    if(d.success) {
      ...
      setR(5, document.querySelectorAll('.r-btn')[4]);
      setTimeout(()=>st.style.display='none', 5000);
    } else {
      st.innerHTML='<div class="alert alert-err">❌ '+(d.error||'Failed')+'</div>';
    }
  } catch(e) {
...
Classify this task as either "crystal" (only the executive can do this) or "bouncy" (can be delegated to an EA or team member).

Crystal Ball tasks: Decisions, negotiations, client relationships, legal/compliance, strategy, approvals — irreplaceable if dropped.
Bouncy Ball tasks: Admin, scheduling, data entry, communication, follow-ups, coordination — recoverable if dropped.
...
Format as a structured 5-day calendar with:
1. Daily Crystal Ball tasks (only the executive can do these)
2. Bouncy Ball tasks delegated to EA
3. Protected time blocks
4. Revenue-generating activities
5. Key metrics to track

Make it actionable, specific, and use the Crystal Ball / Bouncy Ball framework throughout.`;

    const response = await openai.chat.completions.create({
      model: 'gpt-3.5-turbo',
      messages: [
        { role: 'system', content: 'You are an expert executive assistant using the Essential EA methodology. Create detailed, actionable weekly plans.' },
        { role: 'user', content: prompt }
      ],
      temperature: 0.8,
      max_tokens: 1000
...
app.get('/api/history', (req, res) => {
  try {
    ...
    res.json({ success: true, tasks: history, count: history.length });
  } catch (error) {
...
app.post('/api/feedback', (req, res) => {
  try {
    ...
    res.status(500).json({ error: error.message || 'Failed to submit feedback', success: false });
  }
});
...
process.on('SIGTERM', () => {
  console.log('Shutting down...');
  server.close(() => process.exit(0));
});Phase 1 complete - merged frontend into single server.js
