...
function getQBBase() {
  return process.env.QUICKBOOKS_ENVIRONMENT === 'production' ? QB_BASE_PROD : QB_BASE_SANDBOX;
}
...
app.get('/api/quickbooks/status', async (req, res) => {
  try {
    const tokenData = await getQBAccessToken();
    if(!tokenData) return res.json({ connected: false });
    res.json({ connected: true, realmId: tokenData.realmId });
  } catch(e) { res.json({ connected: false }); }
});
...
app.get('/api/quickbooks/profit-loss', async (req, res) => {
  try {
    const tokenData = await getQBAccessToken();
    if(!tokenData) return res.json({ success: false, error: 'Not connected' });
    const startDate = req.query.start || new Date(new Date().getFullYear(), 0, 1).toISOString().split('T')[0];
    const endDate = req.query.end || new Date().toISOString().split('T')[0];
    const url = getQBBase() + '/v3/company/' + tokenData.realmId + '/reports/ProfitAndLoss?start_date=' + startDate + '&end_date=' + endDate + '&minorversion=65';
...
app.get('/api/quickbooks/invoices', async (req, res) => {
  try {
    const tokenData = await getQBAccessToken();
    if(!tokenData) return res.json({ success: false, error: 'Not connected' });
    const url = getQBBase() + '/v3/company/' + tokenData.realmId + '/query?query=SELECT * FROM Invoice WHERE Balance > 0 ORDERBY DueDate DESC MAXRESULTS 20&minorversion=65';
    const r = await fetch(url, {
      headers: { 'Authorization': 'Bearer ' + tokenData.token, 'Accept': 'application/json' }
    });
    const data = await r.json();
...
app.get('/api/quickbooks/expenses', async (req, res) => {
  try {
    const tokenData = await getQBAccessToken();
    if(!tokenData) return res.json({ success: false, error: 'Not connected' });
    const url = getQBBase() + '/v3/company/' + tokenData.realmId + '/query?query=SELECT * FROM Purchase ORDER BY MetaData.CreateTime DESC MAXRESULTS 20&minorversion=65';
    const r = await fetch(url, {
      headers: { 'Authorization': 'Bearer ' + tokenData.token, 'Accept': 'application/json' }
    });
    const data = await r.json();
...
app.get('/api/quickbooks/summary', async (req, res) => {
  try {
    ...
    // Get P&L
    const plUrl = getQBBase() + '/v3/company/' + tokenData.realmId + '/reports/ProfitAndLoss?start_date=' + startDate + '&end_date=' + endDate + '&minorversion=65';
    const plRes = await fetch(plUrl, { headers: { 'Authorization': 'Bearer ' + tokenData.token, 'Accept': 'application/json' } });
    ...
    const invUrl = getQBBase() + '/v3/company/' + tokenData.realmId + '/query?query=SELECT COUNT(*) FROM Invoice WHERE Balance > 0&minorversion=65';
    const invRes = await fetch(invUrl, { headers: { 'Authorization': 'Bearer ' + tokenData.token, 'Accept': 'application/json' } });
    const invData = await invRes.json();
    const unpaidCount = invData.QueryResponse?.totalCount || 0;
